  ...
  async getOther() {
    for (let i = 0; i < this.total/100; i++) {
      await new Promise((resolve, reject) => {
        ...
        this.pagination = body.pagination;
        this.follows.push(body.data);
        process.stdout.write(`\x1b[${6};${this.logger.syswidth + 4}H\x1b[0K ${chalk.hex('#FF00FF').bold('$SYSTEM')} Dumping followers list... ${ Math.floor((i)*100/(this.total/100))}%\x1b[${7};${this.logger.syswidth + 4}H\x1b[0K ${this.pagination.cursor?this.pagination.cursor.slice(10, this.pagination.cursor.length):"finished"}`);
        if (i+1 >= this.total/100) {
          ...
          process.stdout.write(`\x1b[${8};${this.logger.syswidth + 4}H\x1b[0K ${chalk.hex('#FF00FF').bold('$SYSTEM')} Dump ${Timestamp.format(new Date())} is ready in /dumps`);
        }
      }).catch((err) => {
        console.log(err);
      });
  ...
  info() {
    new Promise((resolve, reject) => {
      ...
      this.logger.syslog(this.mod, `| \x1b[32m\x1b[1mSUCCESS!\x1b[0m
            \x1b[${this.logger.syswidth - 1}C└───> \x1b[32mAll stream info successfully received\x1b[0m`);
      Table.build(res, true ,this.logger.syswidth + 2, this.logger.syswidth/2);
      this.logger.canvas();
    }).catch((err)=> {
      console.log(err);
      this.logger.syslog(this.mod,  `| \x1b[31m\x1b[1mERROR\x1b[0m`);
