  ...
  async getOther() {
    for (let i = 0; i < this.total/100; i++) {
      await new Promise((resolve, reject) => {
        ...
        process.stdout.write(`> BOT | [STREAM] Dumping followers list... ${ Math.round((i+1)*100/(this.total/100))}% | ${this.pagination.cursor?this.pagination.cursor:"finished"}`);
        if (i+1 >= this.total/100) {
          ...
          console.log(`> BOT | [STREAM] Dump is ready in /dumps`);
        }
      }).catch((err) => {
        console.log(err);
      });
  ...
  compare(old_d, new_d) {
      ...
      olddump.forEach((oldfollow) => {
        if (!newdump.includes(oldfollow)) {
          this.unfolowed.push(oldfollow);
        }
      });
      console.log(`> BOT | [STREAM] Unfolowed users list :`);
      console.log(`      └> Total unfollowed from ${old_d} to ${new_d} : ${this.unfolowed.length} users`);
      console.log(this.unfolowed);
  };
