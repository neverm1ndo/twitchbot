module.exports = class Party {
  ...
  log(chatter, success) {
    ...
    if (chatter.mod || chatter.subscriber) {
      process.stdout.write(` |${chatter.mod ? '\x1b[1m\x1b[34m MODERATOR\x1b[0m':''}${chatter.subscriber ? '\x1b[1m\x1b[33m SUBSCRIBER\x1b[0m' : ''}`);
    }
  ...
  stack() {
    if (this.chatters.length >= this.amount) {
      ...
      for ( let i = 0; stacked.length < this.amount; i++) {
        let randomchatter = this.random(this.chatters.length);
        if (stacked.length !== 0) {
          let attend = false;
          stacked.forEach((c)=> {
      ...
      return `Нехватило игроков для пати из ${this.amount + 1} человек.`
    }
  }
}
