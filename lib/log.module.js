...
module.exports = class Log {
  ...
  canvas() {
    ...
    this.std.write(`\x1b[1mStream info\x1b[0m`);
    this.std.moveCursor(0, this.lines);
  ...
  print(chatter, timestamp) {
    ...
    this.std.write(chalk.hex(chatter.color).bold(` \x1b[1m${chatter.username}`)+ `\x1b[0m${chatter.username!==''?': ':''}`);
      if ((chatter.message.length + 13 + chatter.username.length) >= this.width ) {
        ...
        // }
      this.std.write(chatter.message.slice(0 , skips - 4) + '...\n');
      } else {
        this.std.write(`${chatter.message}`);
        for (var i = 0; i < (this.width - chatter.username.length - chatter.message.length - 13); i++) {
          this.std.write(' ');
        }
        this.std.write(`\x1b[0m\n`);
      }
  }
  chatlog(chatter) {
  ...
  syslog(mod, message) {
    ...
    if (message.length > 40) message = message.slice(0, 40) + '...';
    this.std.write(chalk.hex(`#FF00FF`).bold(` \x1b[1m$${mod} `) + `${message}\n`);
    // this.canvas();
  }
  playlog(message) {
    this.std.moveCursor(0, this.lines);
    this.std.moveCursor(this.width + 2, -6);
    this.std.cursorTo(this.width + 2);
    if (message.length > (this.width - 10)) message = message.slice(0, this.width - 10) + '...';
    this.std.write(chalk.hex(`#FF00FF`).bold(`$PLAYER `) + `${message}`);
  }
  playerror(message) {
    if (message.length > (this.width/3 - 10)) message = message.slice(0, this.width/1.9 - 10) + '...';
    this.std.write(chalk.hex(`#FF0000`).bold(`\x1b[31m\x1b[${this.lines - 2};${this.width + 4}H${message}`));
  }

  clearSysLog() {
    this.syslogs = 0;
    for (let i = 5; i < this.lines - 20; i++) {
      this.std.write(`\x1b[${i};${this.width + 3}H\x1b[0K`)
    }
  }
  tablelog(table) {
