...
function timeout(delay, path) {
  _timeout = true;
  queue.push(path);
  process.stdout.write(`> BOT | \x1b[1m[ PLAYER ]\x1b[0m : ${path} added to timeout queue\n      └───> QUEUE items [${queue.length}]: [${queue}]\n`);
  setTimeout( t => {
    _timeout = false;
    queue.forEach((p, index) => {
      if ( p == path ) {
        queue.splice(index, 1);
        process.stdout.write(`> BOT | \x1b[1m[ PLAYER ]\x1b[0m : ${path} removed from timeout queue\n      └───> QUEUE items [${queue.length}]: [${queue}]\n`);
      }
    });
  }, delay);
};
...
module.exports = class Player {
  ...
  static play(path, delay) {
    ...
    }).catch((p) => { console.log(`> BOT | [ PLAYER ] ${p} in queue`); })
  }
}
