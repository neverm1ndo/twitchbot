...
module.exports = class Player {
  ...
  global() {
    this._global = true;
    this.logger.playlog(`\x1b[${this.h - 4};${this.w + 3}H Global cooldown enabled\n\x1b[0;0H`);
    setTimeout( t => {
      this._global = false;
      this.logger.playlog(`\x1b[${this.h - 4};${this.w + 3}H Global cooldown disabled\n\x1b[0;0H`);
    }, 80*1000);
  }
  ...
  timeout(delay, path) {
    ...
    this.logger.playlog(`\x1b[0K\x1b[35m${path}\x1b[0m added to timeout queue\n\x1b[${this.w + 3}C\x1b[0K└───> QUEUE audios [${this.queue.length}]: [${this.queue}]\x1b[0;0H`);
    setTimeout( t => {
      this._timeout = false;
  ...
  play(path, delay) {
    new Promise((resolve, reject) => {
      ...
      let audio = player.play(path,  { mplayer: ['-af', `volume=${config.volume.audio}.1:0`]}, (err) => {
        if (err) this.logger.playerror(`${err.message}\x1b[0;0H`);
      });
      mp3d(path, (err, duration) => {
        if (err) return this.logger.playerror(`${err.message}\x1b[0;0H`);
