...
module.exports = class Player {
  ...
  play(path, delay) {
    new Promise((resolve, reject) => {
      ...
      let audio = player.play(path,  { mplayer: ['-af', `volume=${config.volume.audio}.1:0`]}, (err) => {
        if (err) this.logger.playlog(`\x1b[31m\x1b[${this.h - 4};${this.w + 3}H${err.message}`);
      });
      mp3d(path, (err, duration) => {
        if (err) return this.logger.playlog(`\x1b[31m\x1b[${this.h - 2};${this.w + 4}H${err.message}`);
  ...
  video(link, chatter) {
    new Promise((resolve, reject) => {
      if (!this.timeouted.includes(chatter) && !playing) { resolve(chatter) }
      else { reject(chatter) };
    }).then((chatter)=> {
      ...
      ytdl(link).pipe(
        ...
        .on("finish", () => {
          process.stdout.write(`> BOT | \x1b[1m[ PLAYER ]\x1b[0m : \x1b[35m${link}\x1b[0m successfully loaded > `);
            ffmpeg.ffprobe('./probs/clips/substream.mp4', function(err, metadata) {
              process.stdout.write('Playing\n      | Video metadata\n');
              Table.build(metadata.format);
              playing = true;
                player.play("./probs/clips/substream.mp4", { mplayer: ['-af', `volume=${config.volume.video}.1:0`, "-xy", "600"]}, (err) => {
