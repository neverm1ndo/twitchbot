...
const VideoServer = require('../server/omd.media.module');
...
module.exports = class OMD {
  constructor(options) {
    ...
    this.bark = null;
    this.bot = new TwitchBot(environment.bot);
    this.party = {
      gathering: false,
      current: undefined,
    };
  ...
  init() {
    Start();
    this.bot = new TwitchBot(environment.bot);
    this.bark = new Bark(this.userconf, this.opts.schedule.automessages, this.bot);
    ...
    this.bot.on('error', (err) => {
      ...
      Table.build(err);
      console.log('\x1b[31mRestart core module...\x1b[0m');
      this.init();
    });
    this.bot.on('message', async (chatter) => {
      this.readChattersMessage(chatter);
    });
    this.bot.on('subscription', (event) => {
      this.bot.say(`${event.login}, спасибо за подписку, братик! PogChamp. Получай смайлик и возможность ставить свою музыку на стриме!`);
    ...
    this.ws.on('message', (message) => {
      ...
      switch (depeche.event) {
        case 'clip-data': {
          this.bot.say(`/me   ▶ Проигрывается ${JSON.parse(depeche.message).items[0].snippet.title}`);
          break;
        }
        case 'queue-warn': {
          this.bot.say(`ItsBoshyTime ${depeche.chatter}, твоя очередь еще не пришла`);
          break;
        }
        case 'queue': {
          const users = [];
          ...
          this.bot.say(users.length > 0 ? `Кулдаун на воспроизведение клипов у : ${users}` : 'Список пуст.');
          break;
        }
        case 'clip-error': {
          this.bot.say(depeche.message);
          break;
        }
        case 'ytcd': {
          this.bot.say(`@${depeche.message}, осталось еще ${depeche.cooldown}`);
          break;
        }
        case 'ytcd-error': {
          this.bot.say(`@${depeche.message}, ты не на кулдауне`);
          break;
        }
        case 'shutup': {
          if (this.state.status === 'works') {
      ...
      if (chatter.custom_reward_id) {
        ...
        if (chatter.custom_reward_id === '83a50c0d-1051-4e57-a6e7-4d8a3263654c') {
          if (chatter.message.length < 100) {
            ...
            this.bot.say('ItsBoshyTime Слишком много символов. Разрешено максимум 100.');
          }
        }
      }
        ...
        switch (chatter.message) {
          case (chatter.message.includes('!party')): {
            if (this.isPrevileged(chatter) && !this.party.gathering) {
              ...
              if (!amount) amount = 1;
              this.bot.say(`OhMyDog @${chatter.username} собирает пати! Пишите + в чай, если хотите попасть в стак!`);
          ...
          case '!mmr': {
            this.bot.say(`OhMyDog Текущий MMR на мейне: ${this.userconf.mmr}`);
            break;
          }
          case '!roll': {
            this.bot.say(`${chatter.username} нароллил: ${RNG.randomize(0, 101)} BlessRNG`);
            break;
          }
          case '!s': {
            if (this.isPrevileged(chatter) && this.party.gathering) {
              this.party.gathering = false;
              this.bot.say(this.party.current.stack());
            }
            break;
          }
          case '!help': {
            this.bot.say('Вся помощь по командам в описаннии под стримом! OhMyDog');
            break;
          }
          case '!donate': {
            this.bark.donate();
          ...
          case '!players': {
            if (this.party.current.players) { this.bot.say(`Сейчас со стримером играют: ${this.party.current.players}`); } else { this.bot.say('Пати со стримером еще не собиралось.'); }
            break;
          }
          default: { break; }
        }
      ...
      stdin.on('keypress', (str, key) => {
        ...
        if (key.sequence === '\r') {
          if (command.includes('$say')) {
            this.bot.say(command.split('$say')[1].trim());
          }
