...
// const Player = require('./player.module');
...
module.exports = class OMD {
  ...
  init() {
    ...
    this.ws.on('message', (message) => {
      ...
      switch (depeche.event) {
        ...
        case 'shutup': {
          if (this.state.status === 'works') {
            Bot.part(environment.bot.channels[0]);
            this.state.switch();
          }
          break;
        }
        case 'wakeup': {
          if (this.state.status === 'sleeps') {
            ...
            Bot.join(environment.bot.channels[0]);
            this.state.switch();
          }
          break;
        }
  ...
  readChattersMessage(chatter) {
    ...
    if (chatter.message.includes(`${conf.prefix}tts`)) {
      const cut = chatter.message.split(`${conf.prefix}tts`)[1];
      if (this.CheckSub(this.ParseBadges(chatter.badges)) || Queue.checkWhitelist(chatter.username)) {
